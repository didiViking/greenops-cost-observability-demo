...
This repository demonstrates an end-to-end **GreenOps observability pipeline** using **Azure Cost Management exports**, **SPRUCE**, **VictoriaMetrics**, and **Grafana**.
...
- Azure Cost Management Export (Legacy CSV)
- SPRUCE
- VictoriaMetrics
- Grafana
...
```
Azure Billing Export (Legacy CSV)
                │
                ▼
...
The Azure Cost Management **Legacy CSV export** is used as the input to SPRUCE. SPRUCE then transforms the billing data into a standardized **FOCUS-compatible Parquet dataset**, making it easier to consume downstream regardless of the original cloud billing format.
...
The Python exporter included in this repository then converts the Parquet data into Prometheus-compatible metrics that are ingested by VictoriaMetrics and visualized in Grafana.
...
The CSV generated by Azure is used as the input for SPRUCE.

> **Note**
> During development I also experimented with Azure FOCUS exports. At the time of writing, the Legacy export was the format that successfully completed the end-to-end workflow described in this repository.

---

## 2. Process the billing export with SPRUCE

Run SPRUCE against your Azure billing export to generate a normalized Parquet dataset.
...
This Parquet dataset is then consumed by the Python exporter included in this repository.
